...
var EMAIL_TEMPLATE = `<h1 id="title">
  {{ TEMPLATE_TITLE }}
</h1>
<table border="1">
  <tbody>
...
const TEMPLATE_TITLE = ""; // Eg, "New Form Submission" - if blank by default this will be the form name.
const EMAIL_SUBJECT = ""; // Eg, "New Form Submission" - if blank by default this will be 'New ' + TEMPLATE_TITLE + ' submission'

// Comma separated list of email recipients
const PDF_EMAIL_RECIPIENTS = "name@yourcompany.com.au"; // Eg: "name@yourcompany.com.au, name2@yourcompany.com.au"
...
const FORM_URL = "https://docs.google.com/forms/d/1MlepNg0G4Qbcbh6sj9WswUiIj0S0Cw0Nqz5norVvavI/edit"; // Eg, "https://docs.google.com/forms/d/1nkI-u4ENTthkTgKD_wC2wWHuek6GZp7bzDd-BWR2eoc/edit"

// Should we include answers with blank responses in the pdf and email body?
const SKIP_BLANK_ANSWERS = false;

// Should the submitter receive a copy of the email?
const SEND_EMAIL_TO_SUBMITTER = true;

//Should an ID be auto generated for each response?
const AUTO_GENERATE_ID = true;

//What should the ID field be called? (if required)
const SUBMISSION_ID_FIELD_NAME = "Submission ID";
...
function onFormSubmit(e) {
  ...
  if (AUTO_GENERATE_ID === true){
    ...
    // If no column exists, create a new column at end of sheet and update subIndex
    if (subIndex === -1) {
      var lastCol = sheet.getLastColumn();
      sheet.insertColumnAfter(lastCol);
      sheet.getRange(1, lastCol + 1).setValue(SUBMISSION_ID_FIELD_NAME);
      subIndex = lastCol; 
    }
    
    // Set ID number in the correct column for the current form submission
    sheet.getRange(sheet.getLastRow(), subIndex + 1).setValue(subId);
  }
  ...
  // Generate the PDF blob directly in memory using HtmlService
  var pdfBlob = HtmlService.createHtmlOutput(pdfHtmlBody)
    .getAs('application/pdf')
    .setName(template_title + '.pdf'); // It is good practice to explicitly name the attachment
  

  var emailRecipients = PDF_EMAIL_RECIPIENTS

  // Append submitter email if required
  if (SEND_EMAIL_TO_SUBMITTER === true){
    var emailIndex = firstRowValues.indexOf("Email address");
    var submitterEmail = sheet.getRange(sheet.getLastRow(), emailIndex + 1).getValue(); // Use getValue() instead of getValues() for a single cell
    emailRecipients += ", " + submitterEmail;
  }

  // Send the email with the memory-stored PDF
  MailApp.sendEmail(emailRecipients, email_subject, "", {
    htmlBody: htmlBody,
    attachments: [pdfBlob]
  });
}
